...
const supCode = computed(() => route.params.supCode || "");
...
function goView(block) {
  if (block.activeTab === "counsel") {
    router.push({ name: "review", params: { sup_code: block.sup_code } });
  } else if (block.activeTab === "plan") {
    router.push({
      name: "SupportPlan",
      params: { supportCode: block.sup_code },
    });
  } else if (block.activeTab === "result") {
    router.push({
      name: "SupportResult",
      params: { supportCode: block.sup_code },
    });
...
function goRegister(block) {
  // 등록은 각 유형의 등록 화면으로 이동만 담당 (실제 등록은 해당 페이지에서 처리)
  goView(block);
}
  ...
  <div class="container-fluid py-4">
    ...
    <div class="card shadow-sm mb-3">
      <div class="card-body">
        ...
        <div v-else-if="!target" class="text-muted">지원대상 정보를 찾을 수 없습니다.</div>
    ...
    <div
      v-else-if="!loading && supports.length === 0"
      class="text-muted"
    >
      지원 이력이 없습니다.
    </div>
      ...
      >
        ...
        <div class="card-body psw-block-body">
          ...
          <template v-if="block.activeTab === 'counsel'">
            ...
            <div v-if="!block.counsels || block.counsels.length === 0" class="text-muted small">
              등록된 상담일지가 없습니다.
            </div>
          ...
          <template v-else-if="block.activeTab === 'plan'">
            ...
            <div v-if="!block.plans || block.plans.length === 0" class="text-muted small">
              등록된 지원계획이 없습니다.
            </div>
            <div v-else class="psw-entry-list">
              <div
          ...
          <template v-else>
            ...
            <div v-if="!block.results || block.results.length === 0" class="text-muted small">
              등록된 지원결과가 없습니다.
            </div>
            <div v-else class="psw-entry-list">
              <div
              ...
              >
                ...
                <div v-if="result.result_updday" class="text-muted text-end small mt-1">
